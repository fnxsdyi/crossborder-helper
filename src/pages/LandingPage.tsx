...
function BuyoutCta({ user, onEnterApp, amount, licenseKeyPrefix, registerHref }: {
  user: { id: string } | null
  onEnterApp?: () => void
  amount: number
...
export function LandingPage({ onEnterApp, onMemberLogin }: LandingPageProps) {
  ...
  // FlowingPulse referral detection (from post-purchase email link)
  const [referralSource, setReferralSource] = useState<string | null>(null)
  const [referralCode, setReferralCode] = useState<string | null>(null)

  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    setReferralSource(params.get('ref'))
  ...
  return (
    <div className="min-h-screen bg-[#0a0e27] scroll-smooth">
      ...
      <header className="relative z-20 border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-gradient-to-br from-cyan-500 to-cyan-400 rounded-lg flex items-center justify-center shadow-lg shadow-cyan-500/20">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-[length:60px_60px] opacity-[0.03]" style={{backgroundImage: 'linear-gradient(rgba(34,211,238,0.3) 1px, transparent 1px), linear-gradient(90deg, rgba(34,211,238,0.3) 1px, transparent 1px)'}} />
        <div className="absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full bg-gradient-to-br from-cyan-500/10 to-transparent blur-[80px]" />
        ...
        <div className="max-w-6xl mx-auto px-6 pt-20 pb-24">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="relative z-10 animate-fade-in-up">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 mb-8">
              ...
              <div className="flex flex-wrap gap-4 mb-12">
                <button onClick={onEnterApp} className="inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-cyan-500 to-cyan-400 text-white rounded-xl font-semibold text-sm hover:shadow-xl hover:shadow-cyan-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all">{t('landing.freeTrial')}<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg></button>
              ...
              <div className="flex flex-wrap items-center gap-6">
                <div className="flex items-center gap-2 text-sm text-slate-500"><svg width="16" height="16" viewBox="0 0 24 24" fill="#475569"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> Worldwide</div>
            ...
            <div className="relative animate-fade-in-up-delay">
              <div className="bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-6 shadow-2xl animate-float">
                ...
                <div className="flex gap-1 mb-5"><button className="px-4 py-1.5 bg-cyan-500/20 text-cyan-300 rounded-md text-xs font-medium">Invoices</button><button className="px-4 py-1.5 text-slate-500 rounded-md text-xs hover:bg-white/5">Clients</button><button className="px-4 py-1.5 text-slate-500 rounded-md text-xs hover:bg-white/5">Tax Forms</button></div>
      ...
      <section className="py-24 px-6 bg-slate-950">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="p-8 rounded-2xl border border-cyan-500/20 bg-cyan-500/[0.04] flex flex-col md:flex-row gap-6 items-start md:items-center mb-5">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-cyan-500/20 shrink-0">
              <heroFeature.icon size={26} className="text-cyan-300" />
            </div>
            <div className="flex-1">
              <h3 className="font-semibold text-white text-lg mb-1">{t(heroFeature.titleKey)}</h3>
              <p className="text-sm text-slate-400 leading-relaxed">{t(heroFeature.descKey)}</p>
            </div>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {restFeatures.map((f, i) => (<div key={i} className="p-6 rounded-xl border border-white/5 bg-white/[0.03] hover:border-cyan-500/30 hover:bg-white/[0.05] transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-cyan-500/10"><div className="w-11 h-11 rounded-xl flex items-center justify-center mb-4 bg-white/5"><f.icon size={20} className="text-slate-300" /></div><h3 className="font-semibold text-slate-200 mb-2">{t(f.titleKey)}</h3><p className="text-sm leading-relaxed text-slate-500">{t(f.descKey)}</p></div>))}
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 bg-slate-950">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-3xl mx-auto">
            <h2 className="text-4xl font-extrabold text-white mb-4">As featured on Product Hunt</h2>
            <p className="text-slate-400 mb-10">TaxFlow launched on Product Hunt. See what the maker community had to say.</p>
            <div className="bg-gradient-to-br from-cyan-500/10 to-cyan-500/10 border border-cyan-500/20 rounded-2xl p-8 flex flex-col items-center justify-center text-center">
              <div className="mb-4">
      ...
      <section id="pricing" className="py-24 px-6 bg-[#0a0e27]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            ...
            <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-6 relative"><h3 className="text-lg font-semibold text-white mb-1">{t('landing.proPlan')}</h3><p className="text-slate-400 text-sm mb-4">{t('landing.pricingDesc')}</p><div className="flex items-baseline gap-1 mb-1"><span className="text-3xl font-bold text-white">${getTaxflowPrice()}</span><span className="text-sm text-slate-500">one-time</span></div><ul className="space-y-2 mb-6 text-sm">{[1,2,3,4,5,6,7].map(i => (<li key={i} className="flex items-center gap-2 text-slate-400"><Check size={14} className="text-green-500 flex-shrink-0" />{t('landing.proFeature' + i as never)}</li>))}</ul><BuyoutCta user={user} onEnterApp={onEnterApp} amount={getTaxflowPrice()} licenseKeyPrefix="TAXFLOW-LIFETIME" registerHref="/register" /></div>
            {referralSource === 'flowingpulse' ? (
              ...
              <div className="bg-gradient-to-br from-cyan-500 to-cyan-600 rounded-2xl p-6 text-white relative overflow-hidden"><div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -mr-8 -mt-8" /><span className="inline-block px-2 py-0.5 bg-green-500 text-white text-[10px] font-bold rounded mb-2 uppercase tracking-wide">Founder</span><h3 className="text-lg font-semibold mb-1">Founder's Edition</h3><p className="text-white/70 text-sm mb-4">Founding-member rate — locked for life. Limited to first 100.</p><div className="flex items-baseline gap-1 mb-1"><span className="text-3xl font-bold">${getTaxflowPrice(TAXFLOW_LAUNCH_CODE)}</span><span className="text-sm text-white/70">one-time</span></div><p className="text-xs text-white/60 mb-4">Founding rate, locked for life. One payment — own TaxFlow forever. No subscription, no recurring fees.</p><ul className="space-y-2 mb-6 text-sm">{[1,2,3,4,5,6,7].map(i => (<li key={i} className="flex items-center gap-2 text-white/90"><Check size={14} className="text-green-300 flex-shrink-0" />{t('landing.proFeature' + i as never)}</li>))}</ul><BuyoutCta user={user} onEnterApp={onEnterApp} amount={getTaxflowPrice(TAXFLOW_LAUNCH_CODE)} licenseKeyPrefix="TAXFLOW-LIFETIME" registerHref="/register" /><p className="text-xs text-white/60 text-center mt-3">Claim your founder rate</p></div>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-6 bg-[#0a0e27] border-t border-white/5 text-center">
        ...
        <p className="text-slate-400 mb-8 max-w-md mx-auto">Join freelancers worldwide who use TaxFlow to invoice and manage US taxes across borders.</p>
        <button onClick={onEnterApp} className="inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-cyan-500 to-cyan-400 text-white rounded-xl font-semibold text-sm hover:shadow-xl hover:shadow-cyan-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all">{t('landing.freeTrial')}<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg></button>
      </section>
