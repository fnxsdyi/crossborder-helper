...
export async function checkW8BENUsage(userId?: string): Promise<{
  ...
  try {
    ...
    if (userId) {
      ...
      if (error) {
        console.warn('[TaxFlow] w8ben_usage query failed:', error.message)
      }

      const used = count || 0
      return {
        allowed: used < W8BEN_FREE_LIMIT,
        used,
        limit: W8BEN_FREE_LIMIT,
        hasSubscription: false,
      }
    } else {
      const usage = getGuestUsage()
      return {
        allowed: usage.count < W8BEN_FREE_LIMIT,
        used: usage.count,
        limit: W8BEN_FREE_LIMIT,
        hasSubscription: false,
      }
    ...
    console.error('[TaxFlow] checkW8BENUsage failed:', err)
    return { allowed: true, used: 0, limit: W8BEN_FREE_LIMIT, hasSubscription: false }
  }
}

export async function recordW8BENUsage(userId?: string): Promise<void> {
  try {
    if (userId) {
      await supabase.from('w8ben_usage').insert({
        user_id: userId,
    ...
    console.warn('[TaxFlow] Failed to record W8BEN usage:', err)
  }
}
